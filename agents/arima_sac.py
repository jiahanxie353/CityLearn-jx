...
class ARIMASAC:
    def __init__(self, building_ids,
        ...
        self.expected_demands = {uid: np.array([0.]) for uid in building_ids}
        ...
        self.target_soft_q_net2, self.policy_net, self.soft_q_optimizer1, self.soft_q_optimizer2, self.policy_optimizer, \
        self.target_entropy, self.alpha, self.encoder, self.encoder_reg, self.elec_estimator, self.norm_mean, \
        self.norm_std, self.r_norm_mean, self.r_norm_std, self.norm_mean, self.norm_std, self.r_norm_mean, self.r_norm_std \
            = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}

        for uid in building_ids:
            self.elec_estimator[uid] = LinearRegression()
            ...
            # Policy
            self.policy_net[uid] = PolicyNetwork(state_dim, action_dim, self.action_spaces[uid],
    ...
    def select_action(self, states):
        ...
        for uid, state in zip(self.building_ids, states):
            if explore:
                ...
                state_ = np.hstack(np.concatenate((state_, self.expected_demands[uid])))
                ...
                self.expected_demands[uid] = self.elec_estimator[uid].predict(x_reg.reshape(1, -1))

                actions.append(act.detach().cpu().numpy()[0])

        return np.array(actions, dtype=object), self.expected_demands

    def add_to_buffer(self, states, actions, rewards, next_states, done, expected_demands, expected_demands_next):
        ...
        for (uid, o, a, r, o2) in zip(self.building_ids, states, actions, rewards, next_states):
            ...
            o = np.hstack(np.concatenate((o, expected_demands[uid])))
            o2 = np.hstack(np.concatenate((o2, expected_demands_next[uid])))
            ...
            if self.time_step >= self.start_regression and (
                    self.regression_flag[uid] < 2 or self.time_step % self.regression_freq == 0):
                # Fit regression model for the first time.
                self.elec_estimator[uid].fit(self.reg_buffer[uid].x, self.reg_buffer[uid].y)
        ...
        if self.time_step >= self.start_training and self.batch_size <= len(self.replay_buffer[self.building_ids[0]]):
            ...
            for _ in range(self.update_per_step):
                for uid in self.building_ids:
                    ...
                    with torch.no_grad():
                        # Update Q-values. First, sample an action from the Gaussian policy/distribution for the
                        # current (next) state and its associated log probability of occurrence.
...
agents = ARIMASAC(**params_agent)

state = env.reset()
done = False

action, expected_demand = agents.select_action(state)
...
if __name__ == "__main__":
    for i in range(2000):
        ...
        action_next, expected_demand_next = agents.select_action(next_state)
        agents.add_to_buffer(state, action, reward, next_state, done, expected_demand, expected_demand_next)
