...
class MARLISA:
    def __init__(self, building_ids,
        ...
        for uid, info in building_info.items():
            _coef = info['Annual_DHW_demand (kWh)'] / .9 + info['Annual_cooling_demand (kWh)'] / 3.5 + info[
                'Annual_nonshiftable_electrical_demand (kWh)'] - info['solar_power_capacity (kW)'] * 8760 / 6.0
            self.energy_size_coef[uid] = max(.3 * (_coef + info['solar_power_capacity (kW)'] * 8760 / 6.0),
                                             _coef) / 8760
        ...
        for uid in building_ids:
            ...
            if building_info[uid]['Annual_nonshiftable_electrical_demand (kWh)'] == 0 and self.encoder_reg[uid][
                23] != 0:
    ...
    def select_action(self, states, deterministic=False):
        ...
        _building_ids_next = [self.building_ids[action_order[(i + 1) % len(action_order)]] for i in
    ...
    def add_to_buffer(self, states, actions, rewards, next_states, done, coordination_vars, coordination_vars_next):
        ...
        for (uid, o, a, r, o2, coord_vars, coord_vars_next) in zip(self.building_ids, states, actions, rewards,
                                                                   next_states, coordination_vars,
            ...
            if self.regression_flag[uid] > 1:
                ...
                # Executed during the training phase. States and rewards pushed into the replay buffer are normalized
                # and processed using PCA.
        ...
        if self.time_step >= self.start_training and self.batch_size <= len(self.replay_buffer[self.building_ids[0]]):
            for uid in self.building_ids:
                # This code only runs once. Once the random exploration phase is over, we normalize all the states
                # and rewards to make them have mean=0 and std=1, and apply PCA. We push the normalized compressed
                # values back into the buffer, replacing the old buffer.
            ...
            for _ in range(self.update_per_step):
                for uid in self.building_ids:
                    ...
                    with torch.no_grad():
                        ...
                        # The updated Q-value is found by subtracting the log-prob of the sampled action (proportional
