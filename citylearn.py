    ...
    for building in buildings.values():
        ...
        if building.dhw_heating_device.nominal_power == 'autosize':
            ...
            if isinstance(building.dhw_heating_device, HeatPump):
                ...
                # We assume that the heat pump is always large enough to meet the highest heating or cooling demand
                # of the building
...
def building_loader(data_path, building_attributes, weather_file, solar_profile, carbon_intensity, electricity_price,
    ...
    for uid, attributes in zip(data, data.values()):
        if uid in building_ids:
            ...
            building = Building(buildingId=uid, dhw_storage=dhw_tank, cooling_storage=chilled_water_tank,
                                electrical_storage=battery, dhw_heating_device=electric_heater,
                                cooling_device=heat_pump, save_memory=save_memory)
            ...
            building.sim_results['solar_gen'] = list(
                attributes['Solar_Power_Installed(kW)'] * data['Hourly Data: AC inverter power (W)'] / 1000)
            ...
            for state_name, value in zip(buildings_states_actions[uid]['states'],
                ...
                if value == True:
                    if state_name == "net_electricity_consumption":
                        ...
                        _net_elec_cons_upper_bound = max(
                            np.array(building.sim_results['non_shiftable_load']) - np.array(
                                ...
                                building.sim_results['dhw_demand']) / .8 + np.array(building.sim_results[
                                                                                        'cooling_demand']) + building.dhw_storage.capacity / .8 + building.cooling_storage.capacity / 2)
                        s_low.append(0.)
                    ...
                    elif (state_name != 'cooling_storage_soc') and (state_name != 'dhw_storage_soc') and (
                            state_name != 'electrical_storage_soc'):
    ...
    observation_space_central_agent = spaces.Box(low=np.float32(np.array(s_low_central_agent)),
                                                 high=np.float32(np.array(s_high_central_agent)), dtype=np.float32)
    action_space_central_agent = spaces.Box(low=np.float32(np.array(a_low_central_agent)),
                                            high=np.float32(np.array(a_high_central_agent)), dtype=np.float32)
...
class CityLearn(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.central_agent:
            ...
            for uid, building in self.buildings.items():
                ...
                building_electric_demand = round(
                    _electric_demand_cooling + _electric_demand_dhw + _non_shiftable_load - _solar_generation, 4)
                ...
                self.buildings_net_electricity_demand.append(
                    -building_electric_demand)  # >0 if solar generation > electricity consumption
            ...
            assert len(
                actions) == self.n_buildings, "The length of the list of actions should match the length of the list " \
                                              "of buildings. "
            ...
            for a, (uid, building) in zip(actions, self.buildings.items()):
                ...
                # Getting input actions and establishing associations between the components of the action array and
        ...
        self.net_electric_consumption_no_storage.append(np.float32(
            electric_demand - elec_consumption_cooling_storage - elec_consumption_dhw_storage - elec_consumption_electrical_storage))
        self.net_electric_consumption_no_pv_no_storage.append(np.float32(
            electric_demand + elec_generation - elec_consumption_cooling_storage - elec_consumption_dhw_storage - elec_consumption_electrical_storage))
    ...
    def cost(self):
        ...
        if self.cost_rbc is None:
            ...
            while not done:
                action = agent_rbc.select_action(
                    [list(env_rbc.buildings.values())[0].sim_results['hour'][env_rbc.time_step]])
        ...
        if 'ramping' in self.cost_function:
            ...
            if self.simulation_period[1] - self.simulation_period[0] > 8760:
                cost_last_yr['ramping_last_yr'] = np.abs(
                    (self.net_electric_consumption[-8760:] - np.roll(self.net_electric_consumption[-8760:], 1))[
        ...
        if '1-load_factor' in self.cost_function:
            ...
            if self.simulation_period[1] - self.simulation_period[0] > 8760:
                cost_last_yr['1-load_factor_last_yr'] = np.mean([1 - np.mean(
                    self.net_electric_consumption[-8760:][i:i + int(8760 / 12)]) / np.max(
                    self.net_electric_consumption[-8760:][i:i + int(8760 / 12)]) for i in
                                                                 range(0, len(self.net_electric_consumption[-8760:]),
                                                                       int(8760 / 12))]) / self.cost_rbc_last_yr[
        ...
        if 'average_daily_peak' in self.cost_function:
            ...
            if self.simulation_period[1] - self.simulation_period[0] > 8760:
                cost_last_yr['average_daily_peak_last_yr'] = np.mean(
                    [self.net_electric_consumption[-8760:][i:i + 24].max() for i in
                     range(0, len(self.net_electric_consumption[-8760:]), 24)]) / self.cost_rbc_last_yr[
        ...
        if 'quadratic' in self.cost_function:
            ...
            if self.simulation_period[1] - self.simulation_period[0] > 8760:
                cost_last_yr['quadratic_last_yr'] = (self.net_electric_consumption[-8760:].clip(min=0) ** 2).sum() / \
                                                    self.cost_rbc_last_yr['quadratic_last_yr']
    ...
    def get_baseline_cost(self):
        ...
        if 'average_daily_peak' in self.cost_function:
            ...
            if self.simulation_period[1] - self.simulation_period[0] > 8760:
                cost_last_yr['average_daily_peak_last_yr'] = np.mean(
                    [self.net_electric_consumption[-8760:][i:i + 24].max() for i in
                     range(0, len(self.net_electric_consumption[-8760:]), 24)])
