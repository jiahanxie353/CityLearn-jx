...
class Building:
    def __init__(self, buildingId, dhw_storage=None, cooling_storage=None, electrical_storage=None,
                 dhw_heating_device=None, cooling_device=None, save_memory=True):
        """
        Args:
            buildingId (int)
    ...
    def reset(self):
        ...
        self.current_net_electricity_demand = self.sim_results['non_shiftable_load'][self.time_step] - \
        ...
        if self.dhw_heating_device is not None:
            self.dhw_heating_device.reset()
            self.current_net_electricity_demand += self.dhw_heating_device.get_electric_consumption_heating(
                self.sim_results['dhw_demand'][self.time_step])
        if self.cooling_device is not None:
            self.cooling_device.reset()
            self.current_net_electricity_demand += self.cooling_device.get_electric_consumption_cooling(
    ...
    def terminate(self):
        ...
        if self.save_memory == False:
            ...
            self.net_electric_consumption_no_pv_no_storage = np.array(
                self.net_electric_consumption_no_storage) + np.array(self.electric_generation)
...
class EnergyStorage:
    def __init__(self, capacity=None, max_power_output=None, max_power_charging=None, efficiency=1, loss_coef=0,
                 save_memory=True):
        """
...
class Battery:
    def __init__(self, capacity, nominal_power=None, capacity_loss_coef=None, power_efficiency_curve=None,
                 capacity_power_curve=None, efficiency=None, loss_coef=0, save_memory=True):
        """
        Generic energy storage class. It can be used as a chilled water storage tank or a DHW storage tank
        Args:
            capacity (float): Maximum amount of energy that the storage unit is able to store (kWh)
    ...
    def charge(self, energy):
        ...
        if self.capacity_power_curve is not None:
            ...
            self.max_power = self.nominal_power * (self.capacity_power_curve[1][idx] + (
        ...
        # Discharging
            ...
            if self.power_efficiency_curve is not None:
                ...
                self.efficiency = self.power_efficiency_curve[1][idx] + (
                            energy_normalized - self.power_efficiency_curve[0][idx]) * (
                                              self.power_efficiency_curve[1][idx + 1] - self.power_efficiency_curve[1][
                                          idx]) / (
                                              self.power_efficiency_curve[0][idx + 1] - self.power_efficiency_curve[0][
                                          idx])
                self.efficiency = self.efficiency ** 0.5
